...
def makeplot(rs, ps, outDir, class_name, iou_type, metric):
    cs = np.vstack([
        np.ones((2, 3)),
        np.array([.31, .51, .74]),
        np.array([.75, .31, .30]),
    ...
    areaNames = ['allarea', 'very tiny', 'tiny', 'small', 'medium']
    types = ['C75', 'C50', 'Loc', 'Sim', 'Oth', 'BG', 'FN']
    for i in range(len(areaNames)):
        area_ps = ps[..., i, 0]
        figure_tile = metric + ' ' + areaNames[i]
        ...
        plt.legend()
        # plt.show()
        fig.savefig(outDir + f'/{figure_tile}.pdf', bbox_inches='tight', pad_inches=0.1)
        plt.close(fig)
...
def analyze_results(res_file, ann_file, res_types, out_dir, max_det=100, metric='IoU'):
    ...
    for res_type in res_types:
        ...
        for k, catId in enumerate(catIds):
            ...
            ps[5, :, k, :, :] = (ps[4, :, k, :, :] > 0)
            ps[6, :, k, :, :] = 1.0
            makeplot(recThrs, ps[:, :, k], res_out_dir, nm['name'], iou_type, metric)
        makeplot(recThrs, ps, res_out_dir, 'allclass', iou_type, metric)
...
def main():
    ...
    parser.add_argument(
        '--max_det', type=int, default=1500)
    parser.add_argument(
        '--metric', type=str, default='IoU')
    args = parser.parse_args()
    analyze_results(args.result, args.ann, args.types, out_dir=args.out_dir, max_det=args.max_det, metric=args.metric)
